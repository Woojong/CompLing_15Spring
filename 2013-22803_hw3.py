...
for files in filename:
    f = open(curr_dir + my_dir + files, 'r')
...
word = []
for lines in data:
    lines = re.sub(r"([!\?\"#$%&()*+,-/:;<=>@\[\]^_`{|}~])", r" \1 ", lines) # wrap around punctuation (not included for ' apostrophe, - for hyphen)
    lines = re.sub(r"\s{2,}", r" ", lines) # replacing double space to single space e.g., "  " to " "
    lines = "<s> " + lines + " </s>" # adding <s>, </s> beginning and end of sentence
...
# converting target sentence into dictionary form
target_sentence = "<s> he was there to take complaint about this course </s>"
target_word = target_sentence.split()
target_list = ngram_maker(target_word, 2, False) # list form
target_dict = ngram_maker(target_word, 2) # dictionary form
for target in target_list: # fill bigram counts
    target_dict[target] = bi_dict[target] if target in bi_dict else 0

# conditional probability for target_dictionary
target_probdict = cond_prob(target_dict, uni_dict)
try: # for processing non-exist bigram.
    sum([log(x) for x in target_probdict.values()])
except:
    sentence_prob = 0

# smmothing result as count and probability each other
...
target_probdict_GTs = cond_prob(GT_smooth(target_dict, bi_dict, 5, False), uni_dict) # GT smoothing result conditional probability
sentence_GT_prob = exp(sum([log(x) for x in target_probdict_GTs.values()]))


def dict_assort(sort_dict, target): # for sorting result as target sentence order
    result_table = []
    for lists in target:
        result_table.append([lists, sort_dict[lists]])
    return result_table

# save txt file result.
w = open(curr_dir + my_dir + savetext, 'w')
w.write("Simple bigram count\n")
header = ["Bigram", "Simple bigram count"]
w.write(tabulate(dict_assort(target_dict, target_list), header, floatfmt = ".15f"))
w.write("\n")
w.write("Simple bigram probability\n")
header = ["Bigram", "Simple bigram probability"]
w.write(tabulate(dict_assort(target_probdict, target_list), header, floatfmt = ".15f"))
w.write("\n")
...
w.write(tabulate(dict_assort(target_count_ONEs, target_list), header, floatfmt = ".15f"))
w.write("\n")
w.write("Laplace smoothing bigram probability\n")
header = ["Bigram", "Laplace bigram probability"]
w.write(tabulate(dict_assort(target_probdict_ONEs, target_list), header, floatfmt = ".15f"))
w.write("\n")
...
w.write(tabulate(dict_assort(target_count_GTs, target_list), header, floatfmt = ".15f"))
w.write("\n")
w.write("GT bigram probability\n")
header = ["Bigram", "GT bigram probability"]
w.write(tabulate(dict_assort(target_probdict_GTs, target_list), header, floatfmt = ".15f"))
w.write("\n")
